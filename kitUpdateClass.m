function dataStruct = kitUpdateClass(dataStruct)
...
[~,tracksIndxMatrix] = convStruct2MatNoMS(tracks);
if size(tracksIndxMatrix,2) ~= numFrames
...
for iFrame = 1 : numFrames
    ...
    [~,unalignedLoc] = intersect(tracksIndxMatrix(:,iFrame),unalignedIdx);
    [~,laggingLoc] = intersect(tracksIndxMatrix(:,iFrame),laggingIdx);
...
if ~isempty(sisterList) && ~isempty(sisterList(1).trackPairs)
    ...
    [~,sistersUnaligned] = intersect(sisterTracks,tracksUnaligned);
    [~,sistersLagging] = intersect(sisterTracks,tracksLagging);
