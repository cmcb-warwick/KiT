function [parameters,sigmaParameters,Q,chiSquared,degreesOfFreedom,residualImage,resAndGauss] = GaussFitND(intensities, coordList, fitParameters, parameters, isNormed)
...
% Fit options.
persistent fitOptions;
if isempty(fitOptions)
  fitOptions = optimset('Display','off','Jacobian','on',...
                        'TolFun',1e-20,'TolX',1e-3);
end
...
if doStatistics
  ...
  [residuals, jacobian, gaussian] = ...
      GaussFitND_lsqnonlinFitFcn(...
        parameters(xIdx),fitParameters,parameters,xIdx,intensities,coordList,...
        isNormed,nDims,isSxy);
end
...
if doStatistics
    ...
    sigmaParameters = nan(size(parameters));
